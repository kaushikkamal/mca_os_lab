...
{
    int *temp_BT;
    int i, target = 0, index, largest_BT, current_time = 0, prev;
    ...
    while (target < p->process_number)
    {
        ...
        for (i = 0; i < p->process_number; i++)
        {
            ...
            {
                if (temp_BT[i] > largest_BT)
                {
                    largest_BT = temp_BT[i];
                    index = i;
                }
            }
        }
    ...
    avg_WT = avg_WT / p->process_number;
    avg_TAT = avg_TAT / p->process_number;
    printf("\n");
